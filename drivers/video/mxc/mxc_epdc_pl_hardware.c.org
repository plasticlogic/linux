			      ...
			      const struct mxc_epdc_plhw_config *conf)
{
	...
	for (i = 0; i < conf->psu_n; ++i) {
		struct pl_hardware_psu *psu = &p->psu;
...
{
	struct pl_hardware_psu *psu;
	printk("PLHW: mxc_epdc_pl_hardware_enable\n");
	if (!p->init_done)
		return -EINVAL;
	psu = &p->psu;
...
{
	struct pl_hardware_psu *psu;
	printk("PLHW: mxc_epdc_pl_hardware_disable\n");
	if (!p->init_done)
		return -EINVAL;
	psu = &p->psu;
	STEP(pl_hardware_cpld_switch(p, CPLD_COM_SW_CLOSE, false),"COM open");
...
{
	int stat = 0;
	printk("PLHW: pl_hardware_cpld_switch: %i\n", sw);
	switch (sw) {
	case CPLD_HVEN:{
